...
contract DirectGrantsSimpleStrategy is BaseStrategy, ReentrancyGuard {
	...
	function __DirectGrantsSimpleStrategy_init(
		uint256 _poolId,
		InitializeData memory _initData
	) internal {
	...
	function setMilestones(
		...
		bool isRecipientCreator = (msg.sender == _recipientId) ||
			_isProfileMember(_recipientId, msg.sender);
	...
	function submitMilestone(
		...
		if (
			_recipientId != msg.sender && !_isProfileMember(_recipientId, msg.sender)
		) {
			revert UNAUTHORIZED();
		}
	...
	function setRecipientStatusToInReview(
	...
	function _registerRecipient(
		...
		if (registryGating) {
			(recipientId, recipientAddress, grantAmount, metadata) = abi.decode(
				_data,
			...
			(recipientAddress, registryAnchor, grantAmount, metadata) = abi.decode(
				_data,
				(address, address, uint256, Metadata)
			);
	...
	function _allocate(
		bytes memory _data,
		address _sender
	) internal virtual override nonReentrant onlyPoolManager(_sender) {
		// Decode the '_data'
		(address recipientId, Status recipientStatus, uint256 grantAmount) = abi
			.decode(_data, (address, Status, uint256));
	...
	/// @param _sender The sender of the distribution
	function _distributeUpcomingMilestone(
		address _recipientId,
		address _sender
	) private {
